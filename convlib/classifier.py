...
class Classifier:
    ...
    def _read_data(self):
        ...
        new_lon = np.linspace(u.longitude[0].values, u.longitude[-1].values, int(u.dims['longitude'] * 0.2))
        new_lat = np.linspace(u.latitude[0].values, u.latitude[-1].values, int(u.dims['latitude'] * 0.2))
        print("*---- Start interp ----*")
